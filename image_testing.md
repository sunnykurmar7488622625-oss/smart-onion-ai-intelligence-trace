...
ONIONAI specifics: POST /api/inspections/analyze accepts {"images": ["data:image/jpeg;base64,...", ...], "source": "CAMERA"|"IMAGE_UPLOAD"} (farmer role only) and returns analysis_id + metrics. Then POST /api/inspections {"analysis_id", "batch_id"} saves it to a batch.
